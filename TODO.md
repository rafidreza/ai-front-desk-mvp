...
- [x] **T37** Voice note ASR (vendor per PRD 08 Q10 benchmark) — **DONE (2026-05-31): added optional voice-note transcription using OpenAI's audio transcription endpoint (`gpt-4o-mini-transcribe` by default), including Messenger direct media URLs, WhatsApp media lookup/download when `WHATSAPP_ACCESS_TOKEN` is configured, stored transcripts for operator review, transcript-aware AI replies, Hono message attachment support, env docs, and regression coverage.**
